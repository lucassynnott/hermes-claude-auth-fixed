...
``sitecustomize.py`` by ``install.sh`` (Linux/macOS) or ``install.ps1``
(Windows).  It runs once at Python interpreter
...
To disable: run ``uninstall.sh`` / ``uninstall.ps1`` or delete the
sitecustomize.py from the venv's site-packages and restart hermes-gateway.
...
if not _PATCHES_DIR or not os.path.isdir(_PATCHES_DIR):
    # Search order: %LOCALAPPDATA%/hermes/patches (Windows) → ~/.hermes/patches
    _candidates = []
    ...
    for _c in _candidates:
        ...
        _PATCHES_DIR = _candidates[-1]  # fallback to ~/.hermes/patches
